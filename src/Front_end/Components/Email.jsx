...
const Showmyticket = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // sendPDFEmail(response.data); // Send the PDF to the email after fetching booking details
    } catch (err) {
      if (error.response && error.response.status === 401) {
        ...
        setError('Error fetching booking details. Please check your booking ID and email.');
      }
    }
    };
// console.log("email",bookingDetails)
    const generatePDF = (details) => {
      if (!details) {
        console.error('Booking details are null or undefined');
        return;
      }
      const doc = new jsPDF();
    ...
    const sendPDFEmail = async (details) => {
      ...
      const accessToken = localStorage.getItem('accessToken');
      try { 
        await api.post('/sendpdfemail', formData, {
          headers: {
    ...
    return (
      <div className='mt-28 p-6 w-full mx-auto bg-gradient-to-b from-gray-200 to-gray-50   space-y-4'>
        ...
        <h2 className='text-4xl font-normal mb-4 text-center'>SMS AND EMAIL TICKET</h2>
        <h2 className='text-xl font-semibold mt-8 text-center'>Verify your details, and <span className='text-red-500'>SMS / EMAIL</span> your tickets</h2>
